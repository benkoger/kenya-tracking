...
def get_class_name_from_filename(file_name):
  ...
  #its a gazelle
  if re.match(r'2017'):
    return [match.groups()[0], 'gazelle_head']
  else:
    print('not gazelle')
    return [match.groups()[0], 'none_of_the_above']
...
def main(_):
  ...
  print('number of eval examples: ', len(val_examples))
  
  logging.info('%d training and %d validation examples.',
               len(train_examples), len(val_examples))

  train_output_path = os.path.join(FLAGS.output_dir, 'kenya_tracking_train.record')
  val_output_path = os.path.join(FLAGS.output_dir, 'kenya_tracking_val.record')
