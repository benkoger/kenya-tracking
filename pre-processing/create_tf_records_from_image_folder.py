...
def _find_image_files(data_dir):
  ...
  #make sure the images are sorted so that they remain in the correct video order
  
  print('unsorted: ', filenames[0:4])
  
  filenames.sort(key=lambda file: (file.split('_')[-4], file.split('_')[-2], int(file.split('.')[-2].split('_')[-1])))
#   filenames.sort(key=lambda file: (int(file.split('.')[-2].split('_')[-1])))

  print('sorted: ', filenames[0:4])
